...
def get_stereo_lookup_table(start_date,end_date,**kwargs):
    ...
    # Check input ranges. STEREO-A is still flying, so its upper bound is "now"
    # (plus a 2-day cushion), not a hardcoded date that silently expires. SPDF's
    # COHO product lags real time by weeks-months; months with no posted CDF just
    # 404 in the monthly download loop and are skipped, so an end_date slightly
    # ahead of available data is harmless.
    stereo_a_max = dt.datetime.now() + dt.timedelta(days=2)
    if (start_date < dt.datetime(2007,1,1) or end_date > stereo_a_max) and spacecraft == 'A':
        print('Error: STEREO-A range must be between 2007.01.01 and {} (now).'.format(
            stereo_a_max.date()))
        return
    ...
    for date in rrule.rrule(rrule.MONTHLY,dtstart=start_date,until=end_date):
        ...
        # Months not yet posted by SPDF (COHO lags real time by weeks-months) 404.
        # Skip them so a refresh fetches as much as is available rather than
        # crashing the whole year on the first missing month.
        try:
            urllib.request.urlretrieve(data_url+data_filename,'stereo.cdf')
        except urllib.error.HTTPError as e:
            print('  {} not available yet ({}), skipping.'.format(data_filename, e.code))
            continue
    ...
    for i, timestamp in enumerate(stereo_data['Epoch']):
        ...
        vlat = stereo_data['lat'][i]
        vlon = stereo_data['lon'][i]
        if np.isnan(vlat): vlat = 0.0
        if np.isnan(vlon): vlon = 0.0
        imf_data['datetime'].append(timestamp)
        imf_data['SatPhi'].append(phi)
        imf_data['Br'].append(br)
        imf_data['Blat'].append(bt)
        imf_data['Blon'].append(bn)
        lat = np.radians(vlat)
        lon = np.radians(vlon)
        ...
        imf_data['n'].append(dens)
        imf_data['T'].append(temp)
