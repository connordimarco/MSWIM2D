...
Production_Scripts/create_solo.py and writes propagated CSVs ready for lookup-table
conversion.
"""
import argparse
import os
...
def solo_ballistic_propagation(raw_data):
    ...
    pd.DataFrame
        ...
        time range, radially scaled to a 1-AU boundary (density and B_r by
        r^2, B_t and B_n by r; speed/temperature unscaled).
    """
    df = raw_data.copy()

    df = df[df['Vr'] > 0]

    # Radial scaling to a 1-AU boundary (frozen-in / Parker spiral). Solar Orbiter
    # measures at r = distance_au (down to ~0.29 AU), where density and field are
    # inflated relative to 1 AU. Without this, the 1-AU lookup-table inner boundary
    # receives values ~1/r^2 too large in density and B_r. Scale each parcel by its
    # OWN measured distance (done here, before distance_au is dropped at the
    # SOLO_OUTPUT_COLS selection, and per-parcel so it travels with the parcel):
    #   density, Br  ~ 1/r^2   (flux conservation; B_r r^2 = const, the Parker flux)
    #   Bt, Bn       ~ 1/r     (azimuthal winds up + transverse; component scaling
    #                           gets |B| magnitude AND direction right since Bt/Br ~ r)
    # Speed is ~constant with r and longitude/timing are handled by the ballistic
    # propagation below, so they are left unscaled. Temperature also falls off with
    # distance (~r^-0.5..-1) but is left unscaled pending a separate decision.
    r_au = df['distance_au']
    df['density'] = df['density'] * r_au ** 2
    df['Br'] = df['Br'] * r_au ** 2
    df['Bt'] = df['Bt'] * r_au
    df['Bn'] = df['Bn'] * r_au
